...
def _build_internal_template(path: Path) -> None:
    """Create a project-owned, dependency-light letter template at runtime.

    The public release intentionally carries no binary letterhead template. Keeping
    the template construction in source code avoids third-party/metadata leakage and
    preserves deterministic shape indexes expected by the renderer.
    """
    prs = Presentation()
