        ...
        try:
            ...
            cur.execute(
                ...
                (world_id, manifest["entity_id"], event_id, created_at,
                 json.dumps(actor, ensure_ascii=False), json.dumps(payload, ensure_ascii=False), payload_hash, policy_ref),
            )
            conn.commit()
    ...
    def wake_entity(self, *, world_id: str, entity_id: str, expected_version: int, actor: dict, policy_ref: str) -> dict:
        ...
        try:
            ...
            cur.execute(
                ...
                (world_id, entity_id, event_id, seq+1, now, json.dumps(actor, ensure_ascii=False),
                 json.dumps(payload, ensure_ascii=False), canonical_json_hash(payload), policy_ref, epoch),
            )
            conn.commit()
