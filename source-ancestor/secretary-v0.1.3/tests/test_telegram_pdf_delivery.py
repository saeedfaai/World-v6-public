...
import tempfile
import pytest
from src.telegram_adapter import TelegramRuntimeConfig, TelegramBotClient
...
def test_secretary_delivery_requires_root_approval():
    s=Secretary001()
    cfg = TelegramRuntimeConfig('TESTTOKEN', '123456789', '123456789')
    with tempfile.TemporaryDirectory() as td:
        p=Path(td)/'letter.pdf'; p.write_bytes(b'%PDF-1.4\nTEST')
        with pytest.raises(PermissionError):
            s.send_pdf_to_root_telegram(p, root_approved=False, config=cfg, opener=fake_opener_ok)
        r=s.send_pdf_to_root_telegram(p, root_approved=True, config=cfg, opener=fake_opener_ok)
        assert r['ok'] is True
