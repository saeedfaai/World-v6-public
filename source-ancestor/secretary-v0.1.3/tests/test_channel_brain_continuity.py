...
from src.telegram_adapter import TelegramRuntimeConfig, normalize_update
...
def test_same_root_identity_across_chatgpt_and_telegram():
    ...
    ctx = s.brain_context()
    assert [x["channel"] for x in ctx] == ["chatgpt", "telegram"]
    assert [x["text"] for x in ctx] == ["اول این کار رو یادداشت کن", "حالا ادامه همون کار"]
...
def test_provider_handoff_does_not_change_conversation():
    ...
    ctx = s.brain_context()
    assert ctx[-2]["brain_provider"] == "openai"
    assert ctx[-1]["brain_provider"] == "google"
    assert len(ctx) == 3
