...
COMPANY = SocietySpec("company", "QUOTE_PROPOSAL", "PURCHASE_APPROVAL", "SUPPLIER_ORDER_EFFECT")
TRADING = SocietySpec("trading", "FORECAST", "TRADE_DECISION", "SYNTHETIC_ORDER")
...
class SharedKernelSocietyAdapter:
    def __init__(self, spec: SocietySpec, seed: int) -> None:
        ...
        i = self.ids
        self.kernel.bind_actor(i["proposer_actor"], "proposer", i["proposer_session"])
        self.kernel.bind_actor(i["approver_actor"], "approver", i["approver_session"])
        self.kernel.bind_actor(i["executor_actor"], "executor", i["executor_session"])
        self.kernel.bind_actor(i["auditor_actor"], "auditor", i["auditor_session"])

    def propose(self) -> None:
        i = self.ids
        self.kernel._append("DOMAIN_PROPOSAL", i["proposer_actor"], i["proposer_session"], "RECORDED", self.spec.proposal_label)

    def approve(self) -> None:
        i = self.ids
        self.kernel.approve(i["approver_actor"], i["approver_session"], i["executor_actor"], i["auth"])
        self.kernel._append("DOMAIN_APPROVAL", i["approver_actor"], i["approver_session"], "RECORDED", self.spec.approval_label)

    def issue_fence(self) -> int:
        i = self.ids
        return self.kernel.issue_fence(i["executor_actor"], i["executor_session"])
    ...
    def effect(self, *, auth: str | None, fence: int | None, expected_version: int,
        ...
        i = self.ids
        s = session or i["executor_session"]
        ok, code, checks = self.kernel.execute(i["executor_actor"], s, auth, fence, expected_version, key or i["key"])
        self.kernel._append("DOMAIN_EFFECT", i["executor_actor"], s, "COMMITTED" if ok else "BLOCKED", f"{self.spec.effect_label}:{code}")
...
INVARIANTS = [
    "proposal_not_authority", "valid_effect", "session_swap_identity",
    "stale_writer_rejected", "duplicate_suppressed", "invalid_fence_rejected",
    "tamper_detected", "restart_reconstructs",
]
...
def run_conformance(spec: SocietySpec, seed: int) -> Dict[str, bool]:
    ...
    a.propose(); a.approve(); fence = a.issue_fence(); new_session = a.swap_executor_session()
    ...
    a.propose(); a.approve(); fence = a.issue_fence()
    ok, _, _ = a.effect(auth=a.ids["auth"], fence=fence, expected_version=a.kernel.version)
    before_effects = list(a.kernel.effects); before_receipts = list(a.kernel.receipts)
    a.restart(); audit_ok, n, actors = a.kernel.reconstruct()
    out["restart_reconstructs"] = ok and a.kernel.effects == before_effects and a.kernel.receipts == before_receipts and audit_ok and n == 1 and actors == {a.ids["executor_actor"]}
...
def run_matrix(trials: int, seed: int) -> dict:
    ...
    for n in range(trials):
        for spec in societies:
            result = run_conformance(spec, seed + n)
            for inv, ok in result.items():
                counts[spec.society][inv] += int(ok)
    rates = {society: {inv: counts[society][inv] / trials for inv in INVARIANTS} for society in counts}
    return {
        "seed": seed,
        ...
        "conformance_vectors_equal": all(rates["company"][inv] == rates["trading"][inv] for inv in INVARIANTS),
...
if __name__ == "__main__":
    ...
    p.add_argument("--output", type=Path, default=Path("results_society"))
    a = p.parse_args(); result = run_matrix(a.trials, a.seed)
