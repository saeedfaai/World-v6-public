...
def cf_stale_fence_after_restart_rotation(seed: int) -> Dict[str, bool]:
    ...
    w = w.restart_runtime(); w.replace_session(i["executor_actor"], i["executor_session_b"])
    ...
    b = b.restart_runtime(); b.bind_session(i["executor_session_b"], "executor")
...
def cf_restart_then_tamper(seed: int) -> Dict[str, bool]:
    ...
    w = w.restart_runtime(); receipts = list(w.receipts)
...
def valid_paths(seed: int) -> Dict[str, bool]:
    ...
    w3 = w3.restart_runtime(); audit_ok, n, _ = w3.reconstruct()
...
def run_gate(trials: int, seed: int) -> dict:
    ...
    for n in range(trials):
        ...
        for key, ok in vr.items(): counters["VALID_PATHS"][key] += int(ok)

    rates = {name: {k: v / trials for k, v in values.items()} for name, values in counters.items()}
    pass_state = all(rates[name]["world8_safe"] == 1.0 for name in rates if name.startswith("CF")) and all(rates["VALID_PATHS"][k] == 1.0 for k in rates["VALID_PATHS"])
    return {
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(); p.add_argument("--trials", type=int, default=1000); p.add_argument("--seed", type=int, default=20260827); p.add_argument("--output", type=Path, default=Path("results_compound")); a = p.parse_args()
    result = run_gate(a.trials, a.seed); a.output.mkdir(parents=True, exist_ok=True)
    (a.output / "compound_fault_gate_v1.json").write_text(json.dumps(result, indent=2, sort_keys=True), encoding="utf-8")
    print(json.dumps(result, indent=2, sort_keys=True))
    if result["gate_state"] != "PASS": raise SystemExit(1)
