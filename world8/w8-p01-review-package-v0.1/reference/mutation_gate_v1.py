...
if __name__ == "__main__":
    p = argparse.ArgumentParser(); p.add_argument("--trials", type=int, default=500); p.add_argument("--seed", type=int, default=20260827); p.add_argument("--output", type=Path, default=Path("results_mutation")); a = p.parse_args()
    result = run_gate(a.trials, a.seed); a.output.mkdir(parents=True, exist_ok=True)
    (a.output / "mutation_gate_v1.json").write_text(json.dumps(result, indent=2, sort_keys=True), encoding="utf-8")
